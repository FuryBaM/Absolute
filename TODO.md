...
- [x] Добавить codegen fuzzer для валидных typed programs с запуском под ASan/UBSan.
  ...
  локальный компилировался, тёк и падал на выходе. Теперь массив владеет
  элементами (раздел 15 в `docs/known-defects.md`), а `copy` массива с
  владеющими элементами отвергается: поверхностная копия удваивала владение.
