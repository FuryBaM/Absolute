...
- [ ] Числовые литералы: `int64` минимум и `uint64` максимум не компилируются,
  наружу утекает `Error: stoll` — голое исключение C++ без файла, строки и
  объяснения. Причина в том, что парсер даёт положительный литерал и отдельный
  унарный минус, поэтому `9223372036854775808` разбирается до применения знака.
  Разбирать надо беззнаково: этот же битовый шаблон и есть минимум `int64`.
- [ ] Не поддержаны формы записи чисел: шестнадцатеричная `0xFF`, двоичная
  ...
  общепринятая запись.
- [ ] `int32 MIN / -1` даёт мусор (наблюдалось `1066133320`). В LLVM это
  неопределённое поведение, и текущая проверка ловит только нулевой делитель.
  Нужно решение: заворачивать как аппаратно или отвергать, — и проверка рядом с
  делением на ноль.
