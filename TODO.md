...
- [x] Добавить codegen fuzzer для валидных typed programs с запуском под ASan/UBSan.
  `tools/testing/codegen_fuzz.py` генерирует целые программы, а не выражения:
  класс с полем-массивом и managed-полем, циклы с вычисляемыми индексами,
  владельцы, создаваемые и удаляемые внутри цикла, рекурсия. Программа
  самопроверяющаяся по построению — она печатает контрольную сумму того, что
  сделала, — поэтому оракулов три, и ни один из них не является второй
  реализацией семантики: санитайзер, проверка утечек и совпадение ответов
  `-O0` и `-O3`.
  Первый же прогон нашёл дефект, который зелёный набор тестов не видел:
  `new T[n]` вообще не помечался как владелец, поэтому локальный массив
  выделялся и никогда не освобождался — в каждой программе. Видно это было
  только на `-O0`: начиная с `-O1` оптимизатор удаляет выделение, которое никто
  не читает обратно. Вторым нашёлся массив, созданный внутри большего
  выражения (`map.toArray().length`): владелец существовал, но его никто не
  забирал. Оба исправлены, регистрация — `absolute.codegen-fuzz`.
  Позже к одной форме программы добавлены ещё четыре: виртуальная диспетчеризация
  через интерфейс, исключения, покидающие кадры с владельцами, замыкания с
  захватом и побегом, дженерики, специализированные под несколько ширин. Первые
  же прогоны дали ещё три дефекта: память массива не обнулялась вопреки
  документации (`new T[n]` звал `malloc`, теперь `calloc`) — видно было именно
  как расхождение `-O0` под ASan с `-O3`; индексное выражение наследовало режим
  доступа всего оператора, из-за чего `a[i % a.length] = v` отвергалось; метод
  дженерик-класса не принимал расширяющие преобразования, которые принимает
  любой другой вызов. Подробности — раздел 14 в `docs/known-defects.md`.
  Ещё четыре формы: свойства и индексаторы (в том числе через интерфейс),
  weak-ссылки под churn, кортежи с `params` и статическими членами, дженерик
  внутри дженерика. Последняя нашла самое крупное: массив никогда не
  освобождал то, чем владеют его элементы, и вручную это тоже было нельзя —
  `delete a[i]` отвергается, как поле такой массив отвергается целиком, а
  локальный компилировался, тёк и падал на выходе. Правка «массив владеет
  элементами» была сделана и **отозвана**: она ломает `Vector<T*>` в трёх
  местах (рост, `pop`, `toArray`), потому что скопированный managed-указатель
  в этом языке — подписчик, а не второй владелец, и тип массива не может
  сказать, что именно он держит. Громкая утечка безопаснее тихого
  преждевременного разрушения, поэтому дефект остаётся открытым, а разбор
  записан в разделе 15 `docs/known-defects.md`. Сохранены две части, верные при
  любом решении: отказ `copy` для массива с владеющими элементами и разбор
  `new T*[n]` в парсере.
  Заодно починен `new T*[n]`: парсер конструктора собирал тип вручную и не
  потреблял `*`, поэтому звезда читалась как умножение — массив владельцев
  можно было создать только литералом с фиксированным числом элементов, то
  есть ровно в том виде, который тёк. Звезда потребляется только когда за ней
  идёт `[`, так что `new Point() * 2` по-прежнему умножение.
  Ещё три формы закрывают оставшиеся слепые пятна: строки, преобразования
  между ширинами и срезы. Строковая — единственная, чьи утечки ожидаемы (у
  строки пока нет времени жизни), поэтому форма объявляет это сама, и проверка
  утечек для неё выключается, а остальные вопросы санитайзера остаются в силе.
  Итого форм четырнадцать; 224 случая одним прогоном — расхождений нет. Это
  отрицательный результат: три новые области под этим оракулом чисты.
- [x] Добавить fuzzing package manifest, lock-file и версий.
  `tests/fuzz/manifest_fuzzer.cpp`: первый байт выбирает цель, поэтому один
  корпус обслуживает три разбора — `SemVer::Parse`/`Satisfies` на буфере
  напрямую, а `LoadManifest` и `LoadLockfile` через временный файл, потому что
  они принимают путь и вызываются компилятором именно так. Отказ исключением —
  это контракт, а не находка, поэтому `std::exception` глотается и до libFuzzer
  доходит только жёсткое падение. За 240 секунд падений нет.
  Попутно нашлась поломка в существующей обвязке: раннер брал бюджет только из
  `--budget-seconds`, а CMake подставляет туда `$ENV{ABSOLUTE_FUZZ_SECONDS}` на
  этапе конфигурации. Значение, выставленное позже — ночным или еженедельным
  прогоном против уже сконфигурированного дерева, — не доходило никуда, то есть
  задокументированное расширение бюджета ничего не расширяло, и добавленный
  ранее CI-job молча гонял бы всегда короткий бюджет. Теперь переменная читается
  и во время запуска: бюджет 20 даёт тест 22 секунды, бюджет 90 — 92.
  Не сделано: plugin manifest (`.absplugin`) и project parser — отдельные
  форматы со своими загрузчиками.
...
- [x] Запускать короткий fuzz budget в PR, расширенный nightly и длительный weekly.
  ...
  Поправка: до сих пор это было отмечено закрытым авансом — механизм бюджета
  существовал, но в CI его никто не вызывал. Ни `run_fuzzer.py`, ни
  `ABSOLUTE_FUZZ_SECONDS` не встречались в воркфлоу, а cron был только ночной.
  Причина, по которой это не бросалось в глаза: libFuzzer поставляется с
  runtime Clang, поэтому CMake создаёт цели только в Clang-сборке — в остальной
  матрице тесты `absolute.fuzz-*` просто не существуют, и их отсутствие ничего
  не роняло. Добавлен job `coverage-guided-fuzzing`, который собирает своё
  дерево Clang'ом и гоняет lexer, parser и analyzer: 60 секунд на цель на push
  и pull request, 300 ночью, 900 в воскресенье; добавлен weekly cron. Корпус и
  reproducer-ы выгружаются артефактом.
  В required gate намеренно пока не включён: сначала нужно несколько прогонов,
  чтобы убедиться в его устойчивости на общих раннерах — блокирующим он должен
  становиться после того, как показал стабильность, а не до.
...
- [x] Проверить compile-time запрет strong cycles и корректность графов с weak
  back-edge. `tests/ownership-torture-weak-backedge.abs`: дерево с сильными
  дугами вниз и weak-дугой вверх, обход замыкается в обе стороны, уничтожение
  корня освобождает все три уровня ровно по разу.
  Формы цикла разнесены по одному файлу на форму. Причина в том, что тест на
  ошибку проходит при любом отказе компиляции: пока три формы лежали в одном
  файле, взаимное владение двух узлов компилировалось и пряталось за соседними
  формами. Оно же и оказалось дефектом — `TransferManagedAliases` переписывал
  владельца у всех алиасов, кроме самого перемещаемого символа, поэтому после
  `left.child = move(right)` у `right` владельцем оставался он сам, и обратная
  дуга `right.child = move(left)` не распознавалась. Программа собиралась и
  текла обоими узлами; ловил это только отчёт рантайма на выходе.
  Форма через алиас отвергается раньше и другим правилом — `move` подписчика
  запрещён, — что зафиксировано в самом файле, чтобы тест не приписывал себе
  диагностику цикла.
...
- [x] Проверить ownership plugin resources и native handles при unload/reload plugin.
  Нашёлся дефект: `PluginManager::Unload` вызывал `shutdown`, закрывал библиотеку
  и вычищал capabilities, но не снимал регистрации плагина. `ResetSyntaxPlugins`
  чистит реестр целиком и вызывается только из деструктора, поэтому при выгрузке
  одного плагина его правила оставались в реестре с raw-указателем `expand`
  внутрь уже выгруженного образа, а ключевые слова — занятыми. Видно это без
  разыменования: повторная загрузка падала с `Syntax keyword 'unless' is already
  registered`, то есть `Reload` был сломан по определению.
  Добавлен `UnregisterSyntaxPlugin(pluginName)`, снимающий правила, opaque-правила,
  бинарные операторы, преамбулы, ресурсы и виртуальные модули этого плагина;
  `Unload` вызывает его до закрытия библиотеки. Преамбулам и виртуальным модулям
  для этого добавлена атрибуция по плагину — у остальных реестров она уже была.
  Проверка в `tests/native/plugin_api_unit_tests.cpp` (load → unload → reload)
  падает без правки и проходит с ней.
  Попутно: этот файл проверял всё через `assert()`, а в Release-сборке CI-матрицы
...
- [ ] Запускать ownership corpus под ASan, UBSan, LSan и TSan, где применимо.
  ...
  managed-memory, scheduler-stress и concurrent-primitives — то есть машинерия
  managed-указателей, где UB опаснее всего. Локально все три чисты; job включён
  в `Hardening required gate` рядом с TSan.
  TSan для сгенерированного кода теперь есть: `--sanitize=thread` ставит
  `ModuleThreadSanitizerPass` и `ThreadSanitizerPass` в конец конвейера и
  линкует `Absolute-Runtime-Tsan` — вторую копию рантайма, собранную под
  санитайзер. Без неё нельзя: TSan рассуждает о программе целиком, и
  неинструментированный планировщик по ту сторону мьютекса превращает каждый
  защищённый доступ в «гонку». Address и thread взаимно исключают друг друга
  (`--sanitize=address --sanitize=thread` отвергается), потому что их shadow
  memory пересекается.
  Под TSan гоняются `task-scheduling`, `task-group`, `concurrency-stress`,
  `async-concurrency-advanced`, `std-concurrent-primitives`,
  `std-typed-channel` — гонок нет. Чистый прогон сам по себе ничего не значит,
  поэтому рядом стоит `tests/thread-sanitizer-race.abs`: программа с
  намеренной гонкой, которая обязана быть найдена. Если она когда-нибудь
  окажется чистой, инструментация перестала работать, и остальные случаи
  ничего не доказывают.
  Попутно нашлась флака: `std-concurrent-primitives` требовал, чтобы два
  читателя перекрылись, но выдерживал их фиксированной задержкой и запускал
  писателя через `delay(2)`. Rwlock отдаёт предпочтение писателю, поэтому
  писатель, успевший встать в очередь между читателями, не пускал второго —
  перекрытие становилось невозможным, и тест падал по причине, не имеющей
  отношения к тому, что он проверяет (5 из 30 запусков под TSan). Теперь
  читатели ждут друг друга, а писатель запускается только после того, как оба
  оказались внутри: 0 из 40 под TSan и 0 из 40 при загруженном CPU.
  UBSan для сгенерированного кода: сначала разобрано, что он вообще нашёл бы.
  Кодоген не ставит `nsw`/`nuw` (арифметика оборачивается, это определено),
  деление и амплитуда сдвига проверяются в рантайме, конверсия float→int
  насыщающая. Единственное, что кодоген добавляет к UB-поверхности, — TBAA, и
  это как раз опасный вид: неверный тег — не медленная программа, а поднятая
  из цикла загрузка, которую запись внутри цикла на самом деле меняла. Ни
  `-O0`, ни санитайзер этого не видят.
  Поэтому вместо фронтендовых проверок сделан `--no-type-alias-info` и
  `tools/testing/alias_differential.py`: та же программа на том же уровне
  оптимизации, с разметкой и без, — 304 программы, расхождений нет.
  Это число ничего не значило, пока проверку не проверили: теги намеренно
  испорчены (уникальный узел на каждое место доступа), и дифференциал ничего
  не нашёл. Причина — разыменование managed-указателя идёт через непрозрачный
  вызов рантайма, который сам по себе сбрасывает предположения о памяти, а
  формы, где тег имеет вес, в наборе не было. Она добавлена в
  `tests/aliasing-guarantees.abs`: два доступа к элементам одного массива в
  цикле дают 800 с честной разметкой и 0 с испорченной, начиная с `-O2`. С ней
  дифференциал ловит подделку на обоих уровнях. Подробности — раздел 16 в
  `docs/known-defects.md`.
  Что остаётся: фронтендовые проверки UBSan (выравнивание, разыменование
  null через raw) — но они дублируют то, что язык уже проверяет сам, поэтому
  пункт держится открытым как утверждение о состоянии, а не как задача.
- [x] Добавить model-based generator операций `new`/alias/weak/move/delete/throw/return
  с эталонной моделью состояния lifetime. `tools/testing/ownership_model.py`.
  Модель независимо ведёт состояние каждого объекта и мест, которые его
  называют, и применяет правила напрямую: alias не владеет и гаснет вместе с
  владельцем, weak не владеет и гаснет вместе с объектом, `move` уводит владение
  и делает источник непригодным, `delete` убивает объект ровно один раз.
  Предсказания модели вкомпилированы в саму программу как проверки: живость
  каждого weak-места и текущий счёт запусков деструктора после каждого шага,
  поэтому расхождение падает на том шаге, где оно возникло, а программа
  сохраняется как reproducer.
  Порождаются только легальные программы: генератор выбирает операцию из того,
  что модель считает сейчас допустимым, поэтому moved-from место не читается и
  подписчик не перемещается. Отклонение неправильных программ — предмет
  error-тестов, а не этого инструмента.
  103 программы до 80 шагов разошлись с моделью ноль раз. Проверено и обратное:
  порча предсказания — как счёта деструкторов, так и живости weak — валит
  сгенерированную программу, то есть проверки действительно связывают.
  `return` и `throw` тоже порождаются: владелец, созданный в вызываемой функции,
  приезжает во владение места вызывающего, а владельцы, живые в кадре, который
  раскручивается исключением, освобождаются раскруткой. Их никто снаружи кадра
  не называет, поэтому счёт деструкторов — единственное наблюдение, и оно тоже
  проверено на связывание: занижение ожидания на двух владельцев валит
  программу. 153 программы до 80 шагов разошлись с моделью ноль раз.
...
- [x] Добавить ABI dump и автоматическое сравнение layout в CI.
  `tests/native/plugin_abi_dump.cpp` печатает размер и выравнивание всех 42
  типов V1 и смещения полей у восьми структур, которые заполняет сам плагин;
  `tests/abi/plugin-abi-v1-lp64.txt` хранит эталон, а
  `tests/compare-abi-dump.cmake` сверяет и при расхождении объясняет, что
  затронуты все уже собранные плагины и что эталон обновляется тем же коммитом,
  если изменение намеренное. Проверка привязана к 64-битной сборке, потому что
  layout от разрядности зависит.
  Смысл именно в автоматике: изменение размера или смещения не видно на уровне
  исходника — плагин вкомпилировал старый layout в свой объектный код, и
  расхождение всплывает падением в чужом бинаре. Проверено обратное: добавление
  поля в `AbsoluteResourceDescriptorV1` роняет тест.
  Не сделано: сравнение экспортируемых символов — это отдельный механизм
  (`nm` по собранной библиотеке), и его стоит делать вместе с выпуском SDK.
...
- [x] Добавить CLI file indexer с `std.fs`, JSON и parallel workers.
  `examples/integration/file-indexer.abs`. Программа герметична: сама создаёт
  дерево под рабочим каталогом, индексирует его четырьмя параллельными
  задачами, сверяет результат со счётом, накопленным при генерации, печатает
  JSON-отчёт, читает его обратно и удаляет дерево. Ничего не зависит от того,
  что лежит на диске, поэтому прогон детерминирован.
  Форма параллельной части продиктована моделью изоляции, а не удобством:
  async-параметр не может быть `string` («task context cannot own its lifetime
  safely»), поэтому воркеры принимают числа и восстанавливают пути по тому же
  соглашению об именах, что и генератор, а отчитываются через общий массив,
  адресуемый по шарду. Задачи нельзя копировать, поэтому они не в массиве, а
  именованные, и все четыре запускаются до первого `await`.
...
- [x] Поддержать переменные уровня модуля в кодогенерации. Хранилище на уровне
  ...
  использование вместо необъявленного хранилища. Добавлен `DeclareGlobalScalar`;
  `FindVariable` уже умел искать в `globals`, так что не хватало именно
  регистрации. Покрыто `tests/global-variables.abs`: чтение из `main`, из
  обычной функции и из async-кадра, изменяемый глобал через несколько вызовов,
  все примитивные формы включая строку и отрицательное значение.
  Владельцы на уровне модуля отвергаются явным сообщением, а не выпускаются:
  хранилище там создаётся один раз и никогда не уничтожается, поэтому у
  managed-указателя не определены ни момент вызова деструктора, ни порядок
  относительно других глобалов. Зафиксировано `tests/global-owner-errors.abs`.
  Отказ перенесён в анализатор с кодом `E_MODULE_SCOPE_OWNER`, поэтому
  сообщение несёт файл, строку и колонку, как того требуют критерии готовности
  языка; в кодогенерации он оставлен подстраховкой. Проверка сознательно узкая
  — только managed, weak и task: у глобального массива хранилище статическое и
  деструктор планировать нечего, поэтому массивы как работали, так и работают.
  Первая, более широкая редакция через `TypeOwnsResources` отвергала и их и
  уронила семь тестов.
  Поправка к тому, что было записано здесь раньше. Утверждалось, что структура
  с собственным `destroy()` на уровне модуля проходит, а деструктор её не
  вызывается. Это не было проверено и оказалось неверным: она не объявляется
  вовсе. Модульные объявления принимают только константные примитивные
  инициализаторы (`GlobalConstant`), а структуре нужен вызов конструктора,
  поэтому объявление читается как исполняемый оператор верхнего уровня и
  отвергается. Тихой порчи здесь нет и деструктор планировать не для чего.
  Оба оставшихся не-дефекта закрыты. Объявление на уровне модуля теперь
  сообщает о настоящем ограничении, с файлом, строкой, колонкой, именем
  переменной и её типом, а не о разборе; сообщение бэкенда о неконстантном
  инициализаторе говорит о константном литерале, потому что `1 + 2` там никто
  не сворачивает. Само ограничение описано в `docs/module-scope.md`: что
  принимает уровень модуля, что отвергает и почему, и куда девать остальное.
- [x] Дописать недостающие формы числовых литералов и сдвиговые присваивания.
  Не лексировались `0xFF`, `0b1010`, `0o17`, `1_000_000`, `1e3` и `1e-9`, а
  `<<=` и `>>=` не разбирались вовсе, хотя бэкенд уже понижал любое составное
  присваивание. Основания и разделители нормализуются в лексере: всё, что ниже
  по течению, читает текст токена через `std::stoull`, который прочитал бы
  `0xFF` как 0 и остановился на `x` — тихий неверный ответ вместо синтаксической
  ошибки. Показатель степени так не убрать, поэтому определение
  «с плавающей точкой» переехало с «в тексте есть точка» на `IsFloatingLiteral`,
  который знает, что `e` внутри `0xE1` — цифра. Литерал, от которого шаблон
  сопоставил лишь часть (`0xZZ`, `1_`, `1e`, `12abc`), теперь лексическая ошибка
  на своей строке и колонке. `tests/literal-notation.abs`,
  `tests/literal-notation-errors.abs`.
- [x] Вызывать `destroy()` базового класса, а не только самый производный.
  Найдено при разборе расхождения порядка раскрутки с документацией.
  Производный `destroy()` затеняет базовый по тому же ключу метода, а
  синтезированный деструктор брал ровно одну запись, поэтому база, закрывающая
  handle или освобождающая нативную память, переставала это делать, как только
  производный класс объявлял свой хук. Поля базы при этом освобождались, и
  ничего не падало — пропадала только рукописная половина. `EmitClassDestructor`
  идёт по цепочке баз через `declaredMethods` и вызывает каждый хук от самого
  производного к базе, с дедупликацией по link name.
  `tests/destruction-order.abs`.
- [x] Записать порядок уничтожения в документации.
  Раскрутка цепочки владеющих полей от корня и обратный порядок полей внутри
  объекта — не противоречие, а две половины одного правила: сначала собственный
  `destroy()` объекта, затем его поля в обратном порядке объявления. Владелец
  разбирается раньше того, чем владеет, поэтому хук ещё видит свои поля.
  `docs/resource-ownership.md` описывал только вторую половину.
- [x] Обстрелять четыре неисследованные области из `docs/known-defects.md`.
  Метод прежний: маленькие программы с однозначным ответом, значения берутся на
  границах, где неверное поведение обязано проявиться.
  - Арифметика указателей: сама арифметика верна (масштаб по элементу проверен
    на `int64` и на структуре с выравнивающим хвостом, разность в элементах со
    знаком, сравнения, запись по вычисленному адресу, отказ для managed и для
    `void`-указателя). Не было записи, которой пишут обход буфера: `p += n`,
    `p++` и `p[i]`. Первое отвергалось потому, что составное присваивание
    сверяло шаг с типом цели, хотя `p = p + 2` принималось. Все три теперь идут
    через один общий `PointerOffset`; индексатор на типе-указуемом по-прежнему
    выигрывает у `p[i]`. `tests/pointer-arithmetic.abs`.
  - Инстанцирование generic'ов: подстановка держится, а вывод типа — нет.
    Аналайзер называл любой целый литерал `int32` независимо от величины, тогда
    как бэкенд всегда эмитировал его нужной ширины; совпадали они лишь пока
    литерал помещался. `identity(10000000000)` возвращал 1410065408. Литерал
    теперь берёт самый узкий подходящий тип, а литерал, не влезающий в целевой
    тип, — это `E_LITERAL_OUT_OF_RANGE`, вместо тихого нуля.
    `tests/generic-instantiation-widths.abs`, `tests/literal-range-errors.abs`.
  - WASM против нативного: каждый исполнимый тест собран обоими бэкендами и
    сверен по выводу и коду возврата. Две дыры линковки — LLVM переписывает
    `printf("%c")` в `putchar`, а на стороне чтения не хватало `getchar`;
    печать символа не собиралась под wasm, хотя нативно работала. Оставшееся
    расхождение конструктивно: каналы под wasm не блокируют, это записано в
    `docs/wasm-target.md`. `tests/wasm-console-libcalls.abs`,
    `tests/wasm-stdin-eof.abs`.
  - Границы коллекций: контейнеры чисты. Дефект нашёлся под ними — чтение поля
    у значения без адреса (результат property или вызова) не компилировалось.
    Значение теперь копируется во временное, а запись через копию отвергает
    аналайзер с файлом и строкой. `tests/collection-boundaries.abs`,
    `tests/value-place-access.abs`.
- [x] Выбирать знаковую или беззнаковую инструкцию по объявленному типу.
  Найдено систематическим обстрелом числовой семантики: из 24 граничных
  проверок 10 оказались проблемными. Тип LLVM знаковости не несёт, а
  `ApplyBinary` схлопывал имена типов в `int32`/`int64`, поэтому деление,
  остаток, сравнения и правый сдвиг всегда эмитировались знаковыми. Это не
  краевой случай: `4294967295u > 1u` возвращало `false`, `(uint8)200 > 100` —
  тоже `false`, `4294967295u / 2` — ноль. Ошибка молчаливая, ничем не
  закреплённая. Правило намеренно узкое — беззнаковая инструкция только когда
  операнд объявлен `uint` и напротив нет более широкого знакового типа, —
  поэтому смешанная арифметика ведёт себя как прежде.
  `tests/unsigned-arithmetic.abs` берёт значения выше знакового максимума своей
  ширины, потому что ниже него подмена инструкции незаметна.
- [x] Числовые литералы разбираются беззнаково, поэтому доступен весь 64-битный
  диапазон: минимум `int64` записывается как `2^63` под унарным минусом, а у
  максимума `uint64` знакового написания нет вовсе. Раньше оба давали `Error:
  stoll` — голое исключение C++ без позиции; выход за 64 бита теперь сообщает
  об этом внятно.
- [x] Сохранять беззнаковость в типе результата бинарного выражения.
  `CommonType` в анализаторе схлопывал `uint32` в `int32`, поэтому
  `(unsignedValue / 1) as int64` расширялся со знаком и давал −1 там, где то же
  значение через переменную давало 4294967295. Прежний тест это пропускал:
  он делил на 2, а у результата `2147483647` старший бит нулевой и подмена
  незаметна. Правило записано лестницей по представимости и одинаково в
  анализаторе и в кодогенерации: `int64` покрывает любой `uint32` и оставляет
  операцию знаковой, `uint64` не покрывает никто, при равном ранге побеждает
  беззнаковый.
- [x] Передавать типы в составное присваивание. `a /= b` шло через
  перегрузку `ApplyBinary` без имён типов, поэтому у беззнаковых оно
  оставалось знаковым, хотя развёрнутая форма `a = a / b` уже была исправлена:
  `4294967295u /= 2` давало ноль там, где то же деление давало 2147483647.
  Найдено проверкой гипотезы по следам собственной правки, а не новым обстрелом.
- [x] Поддержать составные присваивания сдвига `>>=` и `<<=`. Их не было в
  лексере и парсере, и программа просто не компилировалась. Обе формы теперь
  разбираются и лежат на том же сдвиге, что и развёрнутая запись, а разбор
  списка шаблонных аргументов умеет закрывать `>>=` как два `>` со знаком
  равенства. Закреплено `tests/literal-notation.abs` и
  `tests/shift-boundaries.abs`.
- [x] Отвергать деление наиболее отрицательного значения на −1. Частное
  непредставимо, LLVM оставляет случай неопределённым, и наблюдался мусор
  (`1066133320`). Теперь runtime-проверка рядом с проверкой на нулевой
  делитель, со своим сообщением; для беззнаковой арифметики такого случая нет,
  поэтому там она не выпускается. `tests/division-edges.abs` закрепляет
  соседей, которые обязаны делиться нормально.
- [x] Поддержать формы записи чисел: шестнадцатеричную `0xFF`, двоичную
  `0b1010`, восьмеричную `0o17`, разделители `1_000_000`, экспоненту `1e3`.
  Основания и разделители нормализуются в лексере, потому что весь код ниже
  читает текст токена через `std::stoull` и прочитал бы `0xFF` как ноль.
  Экспоненту так убрать нельзя, поэтому распознавание вещественного литерала
  переехало с «в тексте есть точка» на `IsFloatingLiteral`, который знает, что
  `e` в `0xE1` — цифра. Литерал, от которого шаблон покрывает только часть
  (`0xZZ`, `1_`, `1e`, `12abc`), стал лексической ошибкой на своей строке и
  колонке. Закреплено `tests/literal-notation.abs` и
  `tests/literal-notation-errors.abs`.
- [x] Не усекать целочисленные литералы шире 32 бит. Найдено по дороге и
  существовало до этой работы: литерал разбирался как 64-битный и выпускался
  константой i32, поэтому 2^31 возвращался отрицательным, 2^32 — нулём, а
  10^10 — числом 1410065408. Молча, без диагностики, в локальных переменных,
...
#Сводка открытых дефектов и непрощупанных участков вынесена в
`docs/known-defects.md` — она написана как передача в следующую сессию и
разделяет тихие неверные ответы, отсутствующие возможности и расхождения с
документацией.
...
- [ ] Differential corpus не расходится между native/WASM и optimization levels.
  Корпус расширен: до этого обе differential-проверки гоняли одну-единственную
  форму программы — целочисленный `mix`, `while`, одна ветвь и контрольная
  сумма, — меняя только параметры. Плавающая арифметика и целочисленные края
  не покрывались вовсе, а расхождения native/WASM живут именно там.
  Добавлены две формы: double-арифметика со сравнением и конверсией int/double
  (избыточная промежуточная точность, contraction в FMA, округление конверсии) и
  целочисленные края (направление усечения деления и остатка с отрицательным
  операндом, сдвиги у границы ширины, сужение int64 → int32). Обе печатают
  результат, а не сверяются с посчитанным заранее ответом: их оракул — согласие
  всех конфигураций между собой. Это сознательно не ловит ответ, одинаково
  неверный везде (для этого есть форма с контрольной суммой), зато не ставит
  Python-модель семантики второй реализацией, которая сама может ошибаться.
  `build_case` теперь общий для обоих раннеров, поэтому новая форма попадает
  сразу в оба корпуса. Пункт остаётся открытым: это утверждение о состоянии,
  которое должно держаться, а не разовая задача.

  Добавлен третий корпус — сам набор тестов: `tools/testing/suite_differential.py`
  собирает каждый исполняемый тест из `tests/` на `-O0` и `-O3` и сверяет вывод
  и код возврата, а с `--node`/`--wasm-host` добавляет к сравнению ещё и
  WebAssembly. Оракул тот же — согласие конфигураций между собой. Программа,
  которая отвечает по-разному на двух прогонах одной сборки (`std-random`
  печатает seed), определяется отдельным повторным запуском и исключается, а не
  выдаётся за расхождение.

  Смысл в охвате: сгенерированные корпуса покрывают по одной форме программы,
  а `tests/` покрывает строки, коллекции, владение, исключения, JSON и всю
  стандартную библиотеку. Именно так нашлось расхождение native/WASM в
  `std.text.parseInt` — сгенерированный корпус его не видел вовсе. После
  исправления: 148 программ, `O0` против `O3`, расхождений ноль;
  зарегистрирован как `absolute.suite-differential` (метка `differential`,
  около 7,5 минут).

  Ось WebAssembly у этого корпуса потребовала двух правок в самой обвязке,
  прежде чем начала что-то значить. Раннер печатал журнал модуля после возврата
  из `main`, поэтому программа, завершившаяся через проверку времени
  выполнения — а `exit` в wasm-шиме это `__builtin_trap()`, — теряла всё, что
  успела напечатать, и каждый такой тест выглядел расхождением. И пустой журнал
  сравнивался неравным с отсутствующим из-за одного перевода строки. Обе — про
  обвязку, а не про язык; как именно завершается упавшая проверка на каждой
  цели, теперь написано в `docs/wasm-target.md`.

  Из сравнения исключены тесты, у которых ctest-регистрация только wasm-овая
  (`wasm-fs`, `wasm-full-runtime`, `wasm-net`, `wasm-wasi-services`): каждый
  утверждает то, что даёт wasm-хост, и нативно утверждал бы обратное. По дороге
  нашлась настоящая разница: `std.fs.normalize` считает обратный слэш
  разделителем на wasm и не считает на Linux, где это законный символ имени
  файла. Обе правы для своей файловой системы, и это записано в
  `docs/wasm-target.md`.

  Ось цели теперь включена в самой регистрации теста, а не только доступна из
  командной строки: CMake ищет node и wasm-ld и передаёт `--node`/`--wasm-host`,
  если оба нашлись. До этого зарегистрированный `absolute.suite-differential`
  сравнивал только уровни оптимизации, то есть ось, ради которой корпус и
  добавлялся, в CI не работала ни разу. Заодно исправлен способ, которым это
  удалось не заметить: раннер принимал половину пары флагов, молча пропускал
  сборку wasm для каждой программы и печатал `suite-differential=ok` — прогон,
  которого просили сравнить две цели, сравнивал одну и отчитывался успехом.
  Теперь половина пары — отказ с указанием, чего не хватает.
  Текущее состояние с включённой осью: 151 программа, `O0` против `O3` плюс
  WebAssembly, недетерминированных 1 (`std-random` печатает seed),
  расхождений ноль. Цена — время: без wasm тест шёл около 7,5 минут, с ним
  26 (каждая программа собирается трижды плюс повторный запуск на
  недетерминизм), поэтому таймаут поднят до 3600 секунд. Метка `differential`
  на месте, так что прогон без него исключается одной командой.
...
- [ ] Ownership torture suite проходит под sanitizers. Под ASan набор зелёный на
  Linux и macOS; TSan теперь применяется и к сгенерированному коду
  (`--sanitize=thread` плюс `Absolute-Runtime-Tsan`), остаётся UBSan, а также
  пункты P4, ещё не покрытые тестами. Предыдущая редакция этого пункта утверждала, что
...
- [x] Ошибки компилятора всегда содержат файл, строку, колонку и понятный
  diagnostic code. Не выполнялось ни в одной половине: 87 диагностик из 361,
  которые выдают error-тесты, печатались вообще без места, потому что парсер
  проставляет позицию только верхнеуровневому выражению и верхнеуровневому
  оператору, а вложенный узел её не получал; код же записывался у большинства
  диагностик и не печатался ни у одной.

  Теперь `Report` берёт ближайший объемлющий узел, у которого позиция есть, —
  это не тот же токен, но та же строка, и она есть всегда. Отложенная проверка
  владения (`ValidateDeferredOwnershipCalls`) запоминает место вызова, потому
  что выполняется, когда стек уже пуст. Код печатается в конце сообщения:
  `... [E_SHIFT_OUT_OF_RANGE]`. Оставшимся 135 вызовам `Report` без кода коды
  присвоены.

  Держится проверкой `absolute.diagnostic-audit`
  (`tools/testing/diagnostic_audit.py`), которая прогоняет все error-тесты и
  падает на любой диагностике без файла, строки, колонки или кода. Сейчас:
  361 диагностика, без места — 0, без кода — 0.
