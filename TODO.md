...
- [ ] Запускать ownership corpus под ASan, UBSan, LSan и TSan, где применимо.
  ...
  Что этим НЕ закрыто и почему пункт остаётся открытым: UBSan для
  сгенерированного кода. Его проверки порождает фронтенд, а не модульный
  проход LLVM, поэтому их пришлось бы эмитировать самим — отдельная работа.
