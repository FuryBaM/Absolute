...
- [x] Поддержать переменные уровня модуля в кодогенерации. Хранилище на уровне
  ...
  уронила семь тестов.
  Осталось не закрытым: структура с собственным `destroy()` на уровне модуля
  сейчас проходит, а её деструктор никогда не вызовется — тот же класс задачи,
  но требует решения про порядок уничтожения, а не одной проверки.
