...
- [x] Добавить model-based generator операций `new`/alias/weak/move/delete/throw/return
  ...
  Не покрыто: `throw` и `return` как операции внутри генератора — сейчас
  программа плоская, без вложенных функций и раскрутки; исключения при передаче
  владения проверяются отдельно в `tests/ownership-torture-transfer.abs`.
