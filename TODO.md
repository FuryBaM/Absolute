...
- [ ] Поддержать переменные уровня модуля в кодогенерации либо отвергать их в
  анализаторе. Сейчас `const int32 LIMIT = 6;` вне функции проходит семантику и
  падает на кодогенерации с `LLVM codegen: unknown variable or field 'LIMIT'`.
  Воспроизводится минимально и не зависит от `const` и от `async`: любая
  переменная уровня модуля, прочитанная хоть из `main`, хоть из обычной
  функции, даёт ту же ошибку; локальные работают. То есть либо глобалы надо
  эмитировать, либо анализатор обязан отказывать внятной диагностикой, а не
  пропускать до внутреннего сообщения backend-а. Найдено при написании
  integration project — ровно тем, для чего эти проекты и нужны.
