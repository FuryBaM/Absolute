...
- [ ] Запускать ownership corpus под ASan, UBSan, LSan и TSan, где применимо.
  ...
  Что этим НЕ закрыто и почему пункт остаётся открытым: инструментирован
  C++-рантайм, а не код, который порождает компилятор. ASan подключается
  модульным проходом LLVM, а проверки UBSan порождает фронтенд, поэтому для
  сгенерированного кода их пришлось бы эмитировать самим — отдельная работа.
  TSan для сгенерированного кода упирается в другое: он модульный проход, но
  требует инструментировать программу целиком, то есть нужен отдельный вариант
  рантайма, собранный под TSan, иначе неинструментированная синхронизация даёт
  ложные срабатывания.
