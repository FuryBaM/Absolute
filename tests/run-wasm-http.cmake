...
if(NOT BUILD_STATUS EQUAL 0)
    message(FATAL_ERROR "wasm http build failed (${BUILD_STATUS}):\n${BUILD_OUT}\n${BUILD_ERR}")
endif()

get_filename_component(_wasm_dir "${OUTPUT}" DIRECTORY)
set(RUNNER "${_wasm_dir}/run-wasm-http-runner.js")
file(WRITE "${RUNNER}" "
const fs = require('fs');
...
instantiateAbsoluteWasm(fs.readFileSync(wasmPath), {
  ...
  if (typeof exports.wasm_http_probe === 'function') {
    const code = exports.wasm_http_probe();
    if (code !== 0) {
      console.error('probe failed', code);
      process.exit(2);
    }
  }
  ...
  if (!logs.join('').includes('wasm-http=ok')) {
    console.error('missing println', logs);
    process.exit(6);
  }
  console.log('wasm-http=ok');
...
if(NOT RUN_STATUS EQUAL 0)
    message(FATAL_ERROR "wasm http run failed (${RUN_STATUS}):\n${RUN_OUT}\n${RUN_ERR}")
endif()
