...
namespace Absolute {
    ...
    std::unordered_map<std::string, int> precedence = {
        {"=", 1}, {"+=", 1}, {"-=", 1}, {"*=", 1}, {"/=", 1}, {"%=", 1}, {"&=", 1}, {"|=", 1}, {"^=", 1}, // Ïðèñâàèâàíèå
        {"?", 2}, // Òåðíàðíûé îïåðàòîð (ìåæäó `||` è `=`)
        {"||", 3}, // Ëîãè÷åñêîå ÈËÈ
        {"&&", 4}, // Ëîãè÷åñêîå È
        {"|", 5},  // Ïîáèòîâîå ÈËÈ
        {"^", 6},  // Ïîáèòîâîå èñêëþ÷àþùåå ÈËÈ
        {"&", 7},  // Ïîáèòîâîå È
        {"==", 8}, {"!=", 8}, // Ðàâåíñòâî
        {"<", 9}, {"<=", 9}, {">", 9}, {">=", 9}, // Ñðàâíåíèå
        {"<<", 10}, {">>", 10}, // Ñäâèãè
        {"+", 11}, {"-", 11},  // Ñëîæåíèå, âû÷èòàíèå
        {"*", 12}, {"/", 12}, {"%", 12},  // Óìíîæåíèå, äåëåíèå, îñòàòîê
        {"!", 13}, {"~", 13}, // Ëîãè÷åñêîå ÍÅ, ïîáèòîâîå ÍÅ (óíàðíûå)
        {"++", 14}, {"--", 14}  // Èíêðåìåíò è äåêðåìåíò (ïîñòôèêñ)
    };
    ...
    int GetOperatorPrecedence(const std::string& op) {
        ...
        return (it != Absolute::precedence.end()) ? it->second : -1; // -1 åñëè îïåðàòîð íå íàéäåí
    }
    ...
    // Ïðîâåðêà, âõîäèò ëè çíà÷åíèå â äîïóñòèìûå òîêåíû ïî token_spec
    ...
    std::vector<Token> lexer(const std::string& code)
    {
        ...
        int line = 1;
        int column = 1;
        size_t lastPos = 0; // Îòñëåæèâàåì òåêóùóþ ïîçèöèþ
        ...
        for (std::sregex_iterator i = begin; i != end; ++i) {
            ...
            for (TokenType name : tokenOrder) {
                ...
                if (std::regex_match(value, regex_pattern)) {
                    ...
                    // Îáíîâëÿåì `line` è `column` ïîñëå îáðàáîòêè òîêåíà
                    for (char c : value) {
                        if (c == '\n') {
                            line++;
                            column = 1;
