...
namespace Absolute {
    std::unordered_map<TokenType, std::string> token_spec = {
        ...
        {TokenType::KEYWORD, R"(\b(int8|int16|int32|int64|uint8|uint16|uint32|uint64|float|double|char|bool|string|void|dynamic|auto|if|else|switch|match|case|default|for|while|foreach|in|do|break|continue|new|delete|raw|weak|ref|using|import|namespace|extern|export|return|true|false|null|class|struct|interface|enum|group|this|public|private|protected|sealed|internal|virtual|override|const|static|extension|async|await|spawn|catch|finally|try|throw|defer|yield|get|set|operator|is|as|fn)\b)"},
