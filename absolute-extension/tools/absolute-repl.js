 ...
 *
 * Lives inside absolute-extension so the VSIX package is self-contained
 * (extension activation must not depend on repo-root tools/).
 */
...
const language = require('../server/language.js');

function repoRootCandidates() {
    // Packaged: <extensions>/furybam.absolute-extension-x.y.z/tools
    // Dev:      <repo>/absolute-extension/tools
    return [
...
function findCompiler(explicit) {
    ...
    for (const root of repoRootCandidates()) {
        candidates.push(
            path.join(root, '.absolute', 'build', 'windows-release', 'Release', 'absolutec.exe'),
            path.join(root, '.absolute', 'build', 'msvc-release', 'Release', 'absolutec.exe'),
            path.join(root, 'x64', 'Release', 'absolutec.exe')
        );
    }
    candidates.push('absolutec');
